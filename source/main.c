...
// // Wraps text and centers each line horizontally
// void printWrappedCentered(int startRow, const char* text, int offsetY) {
//     int row = startRow + offsetY;
//     char line[33]; // max 32 chars + null
//     int lineLen = 0;
//     char word[64];
//     int wordLen = 0;
...
//         if (lineLen + wordLen + (lineLen > 0 ? 1 : 0) >= 32) {
//             line[lineLen] = '\0';
//             int col = (32 - lineLen) / 2;
//             printf("\x1b[%d;%dH%s", row, col, line);
//             row++;
